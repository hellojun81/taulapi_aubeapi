...
async function add_field(tableName, fieldName, fieldType) {
  if (!fieldName || !fieldType) {
    return res
      .status(400)
      .json({ message: "Field name and type are required" });
  }
  const sql = `ALTER TABLE ${tableName}  ADD COLUMN ${mysql.escapeId(
    fieldName
  )}${fieldType}`;
...
async function delete_field(tableName, fieldName) {
  ...
  const sql = `ALTER TABLE ${tableName} DROP COLUMN ${mysql.escapeId(
    fieldName
  )}`;
  let result = await executeQuery(sql);
  return result;
