    ...
    try {
        const [rows] = await pool.query(
            `SELECT * FROM filmmakersjobs 
             ...
             AND execute_time > DATE_SUB(NOW(), INTERVAL 10 MINUTE)`
        );

        for (const job of rows) {
            const taskDescription = job.task_description.toString();
            console.log(`Executing job: ${taskDescription} scheduled at ${job.execute_time}`);

            if (taskDescription === 'filmmakers') {
                const result = await filmmakersContoller.postEdit();
                await pool.query(
                    `UPDATE filmmakersjobs 
                     SET is_executed = TRUE, result = ? 
                     WHERE id = ?`,
                    [result.message, job.id]
                );
